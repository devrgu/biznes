<template>
      ...
      <div class="zapis-content">
        ...
        <div class="textarea-block block">
          ...
          <textarea
            v-model="problem"
            ref="problem"
            placeholder="Опишите проблему, вопросы, предложения."
          ></textarea>
        </div>
        <div class="textarea-block block">
  ...
  <div class="message-form">
    ...
    <p class="load-form">{{load}}</p>
    </div>
    <div class="description-model">
      <p>
        Нажимая на кнопку "Отправить заявку" Вы соглашаетесь с политикой
        конфиденциальности
        <a target="_blank" href="https://serikaubakirov.tk/privacy"
          >Серика Аубакирова.</a
        >
      </p>
    </div>
    <div class="main-buttons">
      <div class="zapisatsa-button" @click.prevent="send" @click="">
    ...
    async send() {
      ...
      this.load = "Данные отправляются..."
        ...
        this.$emit("someEvent", this.child);
          ...
          this.error = "Соединение прервано, повторите попытку"
        }
      }
    },
  },
...
<style>
.message-form .error-form{
  color: red !important;
    text-align: center;
}
.message-form .load-form{
